...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    if [ ${use_wordlm} = true ]; then
        ...
        mkdir -p ${lmdatadir}
        cut -f 2- -d" " data/${train_set}/text > ${lmdatadir}/train_trains.txt
        ...
        text2token.py -s 1 -n 1 -l ${nlsyms} data/${train_set}/text \
            | cut -f 2- -d" " > ${lmdatadir}/train_trains.txt
